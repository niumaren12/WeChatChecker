...
pyinstaller --onefile --windowed --icon=app.ico --name WeChatChecker --paths . --collect-all wechat_controller --hidden-import PIL --hidden-import PIL.Image --hidden-import PIL.ImageOps --hidden-import PIL.ImageFilter --add-data "config.json;." --add-data "wechat_ids.txt;." --add-data "tesseract_bundle;tesseract" main.py
if %errorlevel% neq 0 (
    echo [错误] 打包失败
    echo 常见原因：
    echo   - 磁盘空间不足
