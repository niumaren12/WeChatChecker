...
→ OCR 截图搜索框下方 → 多关键字回退匹配 → 鼠标点击下拉项
→ 两级UIA搜索定位弹窗 → OCR 截图弹窗 → 7关键字回退判断状态
→ 关闭弹窗（ESC×3 + 鼠标点击备用 + 两级搜索验证）→ 清空搜索框
```
...
- 等待逻辑 `_wait_with_stop()` 每 0.5s 轮询 `_stop_event`，可被停止信号打断
- **所有 `time.sleep()` 必须改用 `_sleep()`**（内部用 `Event.wait(timeout)` 实现可中断等待），因为 `pytesseract.image_to_data()` 是阻塞 C 调用
...
首次 UIA 搜索成功后存 `self.wechat_window`，后续用 Win32 `IsWindow` 验证 + `ShowWindow`/`SetForegroundWindow` 激活，避免每次重新搜索（4次UIA搜索×每号 = 数秒延迟）。
...
- **等待必须可中断** — `_sleep()` 代替 `time.sleep()`，OCR 阻塞期间停止信号仍可生效
- **`close_popup()` 和 `check_popup_status()` 搜索逻辑必须一致** — 否则导致弹窗残留、连锁假阳性
